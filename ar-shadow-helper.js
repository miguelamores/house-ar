  ...
  AFRAME.registerComponent("auto-shadow-cam", {
    ...
    tick() {
      ...
      for (const el of this.data.targets) {
        ...
        const distanceToPlane = distanceOfPointFromPlane(
          cameraWorldPosition,
          normal,
          sphere.center
        );
        const pointOnCameraPlane = nearestPointInPlane(
          cameraWorldPosition,
          normal,
          sphere.center,
          tempVector
        ...
        camera.near = Math.min(
          -distanceToPlane - sphere.radius - 1,
          camera.near
        );
        ...
        camera.bottom = Math.min(
          -sphere.radius + pointInXYPlane.y,
          camera.bottom
        );
      }
    ...
    tick: function () {
      ...
      if (this.data.light) {
        ...
        if (shadow) {
          ...
          const projectionOfCameraDirectionOnPlane = nearestPointInPlane(
            zeroVector,
            planeVector,
            normal,
            planeVector
          ...
          if (
            ...
            projectionOfCameraDirectionOnPlane
              .normalize()
              .multiply(borderWidth);
